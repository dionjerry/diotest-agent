...
async function pushRecorderSession(session) {
  try {
...
async function pushAnalysisSession(run) {
  try {
    const settings = await loadSettings();
    const rawUrl = settings.connection?.diotestApiUrl;
    const apiKey = settings.connection?.diotestApiKey?.trim();
        ...
        case "analysis.run": {
          ...
          if (result.ok) {
            ...
            const synced = await pushAnalysisSession({
              ...
              title: result.debug.raw_context.title?.trim() || void 0,
