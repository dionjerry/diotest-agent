import { ERROR_MESSAGES } from "@diotest/domain/errors/codes";
...
import { loadSettings, saveSettingsAtomically } from "../adapters/settings/storage";
import { autoSessionName } from "@diotest/engine/ui/sessionNaming";
import { clearRecorderState, persistRecorderState, restoreRecorderState } from "../adapters/worker/state";
import type {
  RecorderActiveState,
...
} from "../adapters/recorder/storage";
...
import type { AnalysisMode, ExtractionContext } from "@diotest/domain/analysis/types";
...
import {
  ...
  saveAnalysisSession
} from "../adapters/sessions/storage";
...
async function testDioTestConnection(apiBaseUrl: string, apiKey: string) {
  ...
  try {
    ...
    const payload = await response.json().catch(() => null) as { ok?: boolean; error?: string; projectId?: string } | null;
