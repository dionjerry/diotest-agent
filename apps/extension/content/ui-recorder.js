// apps/extension/content/ui-recorder.ts
...
function buildLabel(element, action) {
...
function uniqueTexts(elements, fallbackPrefix, maxItems) {
  const values = [];
  for (const element of elements) {
    if (!isVisible(element)) continue;
    const text = sanitizeLabelText(
      element.getAttribute("aria-label") || element.placeholder || element.textContent,
...
async function emit(action, target, extra) {
...
function installRecorderListeners() {
  ...
  document.addEventListener("change", (event) => {
    const target = event.target;
    const action = target instanceof HTMLSelectElement ? "select" : "change";
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.payload?.action === "stop") {
    window.__diotestRecorderState = void 0;
    sendResponse({ ok: true });
    return true;
  }
