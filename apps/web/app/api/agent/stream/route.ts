...
export async function POST(request: NextRequest) {
  ...
  let body: { threadId?: string; organizationId?: string; projectId?: string; content?: string };
  try {
  ...
  const { threadId, organizationId, projectId, content } = body;
  if (!threadId || !organizationId || !projectId || !content) {
    return new Response('Missing required fields', { status: 400 });
  }

  const backendResponse = await fetch(
    `${env.apiBaseUrl}/agent-threads/${threadId}/messages/stream`,
    {
      method: 'POST',
      headers: {
